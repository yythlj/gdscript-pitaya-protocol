...
func SendRequest(route, data, cb):
	...
	m.Route = route
	m.Data = data
	m.SentAt = Time.get_unix_time_from_system()
	m.Callback = cb
	var p = buildPacket(m)
...
func connectServer():
	...
	var _strlen = buf[0]
	var serializerName = buf.slice(1, _strlen + 1).get_string_from_utf8()
	buf = buf.slice(_strlen + 1)
	
	var _routelen = buf[0]
	buf = buf.slice(1) if len(buf) > 1 else []
	var routeDict = {}
	for i in range(_routelen):
		var _routestrlen = buf[0]
		var _route = buf.slice(1, _routestrlen + 1).get_string_from_utf8()
		buf = buf.slice(_routestrlen + 1)
		var _code = buf[0] << 8 | buf[1]
		routeDict[_route] = _code
		buf = buf.slice(2) if len(buf) > 2 else []
		
	message_encoder.SetDictionary(routeDict)
	print("handshake   "+ str(routeDict) + str(serializerName))
	p = packet_encoder.Encode(packet_HandshakeAck, PackedByteArray())
	if p == null:
		return
	...
	heartbeat_thread.start(Callable(self, "sendHeartbeats").bind(heartbeatTimeout))

	handle_thread = Thread.new()
	handle_thread.start(Callable(self, "handleServerMessage"))
	
	repear_thread = Thread.new()
	repear_thread.start(Callable(self, "requestReaper"))
	return null
	
func sendHeartbeats(delta):
	while true:
		var p = packet_encoder.Encode(packet_Heartbeat, PackedByteArray())
		client.put_data(p)
...
func requestReaper():
	while Connected:
		...
		for msgid in wait_cb.keys():
			...
			if Time.get_unix_time_from_system() - msg.SendAt > requestTimeout:
				wait_cb.erase(msgid)
				OnTimeoutMsg(msg)
		#c.pendingReqMutex.Unlock()
